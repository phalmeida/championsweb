...

{
	...
	public function check_userlogin () {
		...
		if ( ! isset( $usuario ) || ! isset( $senha_usuario ) ) {
		...
		if ( empty( $user_id ) ){
			...
			$this->login_error = 'Usuário não cadastrado no sistema.';
		...
		if ( $this->phpass->CheckPassword( $senha_usuario, $fetch['senha_usuario'] ) ) {

			

/*			// Se for uma sessão, verifica se a sessão bate com a sessão do BD

			if ( session_id() != $fetch['session_usuario'] && ! $post ) { 
			...
			if ( $post ) {
				...
				$_SESSION['userdata']['senha_usuario'] = $senha_usuario;

				

				// Atualiza o ID da sessão

				$_SESSION['userdata']['session_usuario'] = $session_id;
				...
				$query = $this->db->query(
					...
					'UPDATE usuario SET session_usuario = ? WHERE id_usuario = ?',
			...
			$_SESSION['userdata']['permissao_usuario'] = $fetch['permissao_usuario'];
			...
			if ( isset( $_SESSION['goto_url'] ) ) {
				...
				if ( $goto_url == '/campeonato/sair/' or $goto_url == '/campeonato/sair' or $goto_url == '/campeonato/login/' or $goto_url == '/campeonato/login') {
	...
	final protected function goto_page( $page_uri = null ) {
		...
		if ( $page_uri ) { 
				...
				if ( $page_uri == '/campeonato/sair/' or $page_uri == '/campeonato/sair' or $page_uri == '/campeonato/login/' or $page_uri == '/campeonato/login') {



					$page_uri = '/campeonato/principal/';
